...
parser.add_argument("-t" , "--advance_time", help="Insert date: [YYYY-MM-DD] to change the softwares date, type [current] to show current time\n'I wish we could travel to the date of next easter'", metavar="", default="")    
...
if args.add:
    if args.product_name == "" or args.buy_price == 0 or args.sell_price == 0:
        print("Please Specify full command:  Example: [blue]--add[/blue] [magenta]-p apple[/magenta] [red]-bp 2[/red] [green]-sp 3[/green]")
    else: 
        try:       
            Classes.ProductManager().update_inventory(pricelist,args.product_name,"",args.buy_price, args.sell_price, 0)
        except KeyError:
...
if args.product_list:
    with open(pricelist, "r", newline="") as File:
        ...
        for row in reader:
            print(f'[magenta]{row["product_name"]}[/magenta], [red]buy_price: €{row["buy_price"]}[/red], [green]sell_price: €{row["sell_price"]}[/green], [blue]Stock: {row["stock"]}[/blue]')       
...
if args.advance_time != "":
    if args.advance_time == "current":
        ...
        print(f"Changed the date to: [blue]{args.advance_time}[/blue].")
