    ...
    def get_data(self, file, colum, search_value="xyz"):
        ...
        with open(file, "r") as File:                                                       
            for row in csv.DictReader(File):                                                
                match colum:
                    case "full_inventory": 
                        data.append([row["product_name"], row["stock"]])
                    case "highest_id":                          
                        highest_id = row["id"]                                           
                    case "revenue":                                               
        ...
        match colum:   
            ...
            case "full_inventory": print(tabulate(data, headers='firstrow', tablefmt='grid', showindex=True))                                                                               
...
class ProductManager():
    ...
    def update_inventory(self, file, product_name, buy_sell, new_buy_price, new_sell_price, quantity):
        ...
        if product_exists == True:
            ...
            if buy_sell == "sell":
                new_stock = int(current_stock) - int(quantity) 
                # Checks if there is enough stuock
                if new_stock < 0:
            ...
            with open(pricelist, "r") as File:
                ...
                for row in product_list:
                    if product_name in row:
                        line_to_override = {product_id:[product_id, product_name, buy_price, sell_price, new_stock]}     
        ...
        if product_exists == False:
            ...
            new_id = FileScanner.get_highest_id(self,pricelist) + 1
            with open(pricelist, "a", newline="") as File:
                writer = csv.writer(File)    
                writer.writerow([new_id, product_name,new_buy_price, new_sell_price, quantity])
